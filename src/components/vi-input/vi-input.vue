    ...
    <template v-if="type !== 'textarea'">
      ...
      <transition name="fade-transition">
        <span
          ...
          v-if="suffixIcon || showClear || showPwToggle">

          <vi-icon v-if="!showClear" class="vi-input__suffix-icon" :name="valid ? 'done' : this.suffixIcon" :size="suffixIconSize"/>

	        <vi-icon v-if="showPwToggle" class="vi-input__suffix-icon" :name="pwVisible ? 'visible-off' : 'visible'" @click.native="pwVisible = !pwVisible" style="cursor: pointer"/>
  ...
  export default {
    ...
    computed: {
      showClear () {
	    ...
	    computedType () {
      	return this.pwVisible ? 'text' : this.type
	    }
    },
    methods: {
      handleBlur (event) {
